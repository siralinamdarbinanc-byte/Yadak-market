...
class ProductViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            repository.syncFromServer() // سینک اولیه، همون لحظه‌ی باز شدن اپ
        }
        startAutoSync()
    }

    // هر ۶۰ ثانیه یه بار تا اپ بازه، از سرور آپدیت می‌گیره
    // (بی‌سروصدا: syncStatus رو تغییر نمی‌ده تا مزاحم کاربر نشه)
    ...
    fun syncFromServer() {
        viewModelScope.launch {
            _syncStatus.value = "در حال دریافت از سرور..."
    ...
    fun pushCatalogToServer() {
        viewModelScope.launch {
            _syncStatus.value = "در حال ارسال به سرور..."
